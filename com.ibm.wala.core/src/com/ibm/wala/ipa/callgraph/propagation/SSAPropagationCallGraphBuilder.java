...
public abstract class SSAPropagationCallGraphBuilder extends PropagationCallGraphBuilder implements HeapModel {
  ...
  // sz: used to store all ssa instruction and its corresponding CGNode and block(since they would change
  // once new IR is built)
  public HashMap<SSAInstruction, ArrayList<Object>> del = new HashMap();
  // sz: record total num of instructions
...
//    System.out.println("       node: " + node.getMethod().toString());
...
//          }
        ...
        if (wasChanged(node)) {
          if(DEBUG){
            System.out.println("--- node is changed! Node: "+node.toString());
            System.out.println("--- and the instruction is: "+ s.toString());
          }
          return;
        }
      }
...
//      if(system.isChange){
//        MutableMapping<PointerKey> pointerkeys = getBuilder().getPointsToMap().getPointerKeys();
//        Iterator<PointerKey> iterator = pointerkeys.iterator();
//        while(iterator.hasNext()){
//          PointerKey key = iterator.next();
//          if(key instanceof LocalPointerKey){
//            LocalPointerKey localkey = (LocalPointerKey) key;
//            if(localkey.getNode().equals(node) && localkey.getValueNumber() == valueNumber){
//              return localkey;
//            }
//          }
//        }
...
//      }
    ...
    // sz: all if-phrase with test
    protected void doVisitArrayLoad(int def, int arrayRef) {

      PointerKey result = getPointerKeyForLocal(def);
      //--- the PointerKey that acts as a representative for the class of pointers that
      //--- includes the local variable identified by the value number parameter.
      //~~~ From Symbol Table
      PointerKey arrayRefPtrKey = getPointerKeyForLocal(arrayRef);
      if (hasNoInterestingUses(def)) {
        //--- FindorCreate first, then set it as implicit
        //--- def(result) = arrayRef(arrayRefPtrKey)
        //~~~ meaning of implicit points to set????
        //~~~ arrayRef is already in system
        if(!isDelete)
...
//            system.delConstraintHasMultiR(result, assignOperator, rhss, delset);//bz: ??efficiency
          }else{
            for (int i = 0; i < ik.length; i++) {
              if (!representsNullType(ik[i])) {
                system.findOrCreateIndexForInstanceKey(ik[i]);
          ...
          rhss.clear();
        } else { // --- if arrayRef contents are variant and def has interesting uses
          //--- used to represent points-to-sets that are unified with another
          //--- result and arrayRefPtrKey cannot be unified with other points to set
          assert !system.isUnified(result);
          assert !system.isUnified(arrayRefPtrKey);
          //--- newSideEffect(UnaryOperator<PointsToSetVariable> op, PointerKey arg0)
          //--- newStatement(null, op, findOrCreatePointsToSet(arg0), true, true);
          if(isDelete)
    ...
    public void doVisitArrayStore(int arrayRef, int value) {
      ...
      //--- arrayRef(arrayRefPtrKey) = value(valuePtrKey)
      ...
      if (contentsAreInvariant(symbolTable, du, arrayRef)) {
        //--- record this pointer key in the points-to set map, set it as implicit
        if(!isDelete)
          system.recordImplicitPointsToSet(arrayRefPtrKey);
        //--- Object x = symbolTable.getConstantValue(valueNumber);
        //--- InstanceKey ik = hm.getInstanceKeyForConstant(type, String (x)/x);
        InstanceKey[] ik = getInvariantContents(arrayRef);
        for (int i = 0; i < ik.length; i++) {
...
//                    int index = system.findOrCreateIndexForInstanceKey(vk[j]);
...
//                  system.delConstraintMultiInstancefromL(p, delset);
                }else{
                  for (int j = 0; j < vk.length; j++) {
              ...
              } else {//--- value contents are variant
    ...
    protected void doVisitArrayLoad_Old(int def, int arrayRef) {
      PointerKey result = getPointerKeyForLocal(def);
      PointerKey arrayRefPtrKey = getPointerKeyForLocal(arrayRef);
      if (hasNoInterestingUses(def)) {
        system.recordImplicitPointsToSet(result);
     ...
     * @see com.ibm.wala.ssa.SSAInstruction.Visitor#visitArrayStore(com.ibm.wala.ssa.SSAArrayStoreInstruction)
     */
    public void doVisitArrayStore_Old(int arrayRef, int value) {
      // (requires the creation of assign constraints as
      // the set points-to(a[]) grows.)
      PointerKey valuePtrKey = getPointerKeyForLocal(value);
      PointerKey arrayRefPtrKey = getPointerKeyForLocal(arrayRef);
    ...
    public void visitReturn(SSAReturnInstruction instruction) {
      ...
      doVisitReturn(instruction);
    }

    public void doVisitReturn(SSAReturnInstruction instruction){
      if (DEBUG) {
        if(isDelete)
      ...
      //---instruction.getResult: value number of the result. By convention result == -1 means returns void.
      ...
      if (contentsAreInvariant(symbolTable, du, instruction.getResult())) {
        if(!isDelete)
          system.recordImplicitPointsToSet(result);
        //---get Invariant Contents: the complete set of instances that the local with vn=valueNumber may point to.
...
//          system.delConstraintMultiInstancefromL(returnValue, delset);
    ...
    public void visitReturn_Old(SSAReturnInstruction instruction) {
    ...
    public void visitGet(SSAGetInstruction instruction) {
      //--- assign a filed of instance to another val
      //~~~ lval = ref.field
      int lval =instruction.getDef();
      int ref = instruction.getRef();
      boolean isStatic = instruction.isStatic();
      FieldReference field =instruction.getDeclaredField();
      visitGetInternal(lval,ref,isStatic,field);
    }



    public void visitGetInternal(int lval, int ref, boolean isStatic, FieldReference field){
      if (DEBUG) {
        if(isDelete)
      ...
      if (hasNoInterestingUses(lval)) {
        ...
        {
          //delete
          //should update symbol table?? remove all occurence of lval from symbol table

          //String v = symbolTable.getValueString(lval);
          //System.out.println(v);//JEFF
        }
      } else {
        if (isStatic) {
          ...
          } else {//--- ref is variant && ldef has interesting uses
            if(isDelete)
     ...
     * @see com.ibm.wala.ssa.SSAInstruction.Visitor#visitGet(com.ibm.wala.ssa.SSAGetInstruction)
     */
    public void visitGet_Old(SSAGetInstruction instruction) {
      visitGetInternal(instruction.getDef(), instruction.getRef(), instruction.isStatic(), instruction.getDeclaredField());
    }

    protected void visitGetInternal_Old(int lval, int ref, boolean isStatic, FieldReference field) {
      if (DEBUG) {
        System.err.println("visitGet " + field);
      }
    ...
    private void processPutField(int rval, int ref, IField f) {
      ...
      if (contentsAreInvariant(symbolTable, du, rval)) {
        ...
        } else {//--- rval is invariant && ref is variant
          for (int i = 0; i < ik.length; i++) {
            system.findOrCreateIndexForInstanceKey(ik[i]);
            if(isDelete)
      ...
      } else {//---rval is variant
        if (contentsAreInvariant(symbolTable, du, ref)) {
          //--- rval is variant && ref is invaraint
          if(!isDelete)
            system.recordImplicitPointsToSet(refKey);
          InstanceKey[] refk = getInvariantContents(ref);
          for (int j = 0; j < refk.length; j++) {
        ...
        } else {//--- rval is variant && ref is variant
          if (DEBUG) {
    ...
    private void processPutStatic(int rval, FieldReference field, IField f) {
      //--- field references set
      PointerKey fKey = getPointerKeyForStaticField(f);
      //--- <node, value number> pair
      PointerKey rvalKey = getPointerKeyForLocal(rval);
      ...
      if (contentsAreInvariant(symbolTable, du, rval)) {
        ...
        if(isDelete){
          ...
          system.delConstraintMultiInstancefromL(fKey, delset);
        }else{
          for (int i = 0; i < ik.length; i++) {
   ...
   // sz: please change test to something else, like enable_incremental_pointer_analysis
      // almost all visitXX has its corresponding DelXX() function
      if(isDelete)
    ...
    /*
     * sz: del a invoke ssa instruction
     */
    protected void doDelInvokeInternal(final SSAAbstractInvokeInstruction instruction, InvariantComputer invs) {
      ...
      //~~~ include variant(null in the array) and invariant parameters
      //*** TODO: what if symbol table changed????, currently, we assume it's not changed
      ...
      if (instruction.getCallSite().isStatic()) {
        ...
        //*** do not actually delete the node, but clear its content
        ...
        if (contentsAreInvariant(symbolTable, du, vns)) {
          for(CGNode n : getBuilder().getTargetsForCall(node, instruction, invariantParameters)) {
            //--- caller, instruction, callee, invariant parameters, unique catch
            getBuilder().processDelCall(node, instruction, n, invariantParameters, uniqueCatch);
            //processClassInitializer(n.getMethod().getDeclaringClass());
    ...
    protected void visitInvokeInternal(final SSAAbstractInvokeInstruction instruction, InvariantComputer invs) {
      ...
      PointerKey uniqueCatch = null;
      try{//JEFF: java.lang.IllegalArgumentException: invalid site
      if (hasUniqueCatchBlock(instruction, ir)) {
        uniqueCatch = getBuilder().getUniqueCatchKey(instruction, ir, node);
      ...
      if(isDelete)
      {
        if (instruction.getCallSite().isStatic()) {
          ...
          for (CGNode n : getBuilder().getTargetsForCall(node, instruction, invariantParameters)) {
            getBuilder().processDelCall(node, instruction, n, invariantParameters, uniqueCatch);
            //TODO: remove call edge from call graph
          }
        }
        else {
   ...
   // sz: all if-phrase with test are new
      ...
      if (DEBUG) {
        System.err.println("SE: visitNew call clinit: " + klass);
      }
      if(!isDelete)
        processClassInitializer(klass);
  ...
  /*
   * sz: Since resolving calls have already been processed, only clear content
   *
   */
  private void processDelCall(CGNode caller, SSAAbstractInvokeInstruction instruction, CGNode target,
    ...
    caller.delTarget(instruction.getCallSite(), target);
  ...
  /*
   * sz: Reduce the calling constraints to pk<->pk statement in flow graph and add steps in worklist
   *
   */
...
//    caller.delTarget(instruction.getCallSite(), target);
      //--- skip all those validations
    for (int i = 0; i < instruction.getNumberOfParameters(); i++) {
      if (target.getMethod().getParameterType(i).isReferenceType()) {//--- sloppy filter

        PointerKey formal = getTargetPointerKey(target, i);
        if (constParams != null && constParams[i] != null) {//--- some variant parameters are included in this array
...
//          }else{
//            for (int j = 0; j < ik.length; j++) {
//              int index = system.findOrCreateIndexForInstanceKey(ik[j]);
//              delSet.add(index);
//            }
//            system.delConstraintMultiInstanceL(formal, delSet);
//          }
//          counter++;
        } else {//--- for those variant parameters
...
//    System.out.println("edges: "+ counter);
...
//          if(ik.length < 5){
            for (int j = 0; j < ik.length; j++) {
              system.newConstraint(formal, ik[j]);// para do not consider this
            }
//          }else{
//            MutableIntSet addSet = IntSetUtil.make();
//            for (int j = 0; j < ik.length; j++) {
//              int index = system.findOrCreateIndexForInstanceKey(ik[j]);
//              addSet.add(index);
//            }
//            system.addConstraintMultiInstancetoL(formal, addSet);
//          }
        } else {
          if (instruction.getUse(i) < 0) {
            Assertions.UNREACHABLE("unexpected " + instruction + " in " + caller);
    ...
    // }
//    System.out.println("edges: "+ counter);
    //collect all added edges
//    system.checkScenarioAmongEdges(lhss, rhss);
  }
  ...
  final class DispatchOperator extends AbstractOperator<PointsToSetVariable> implements IPointerOperator {
    ...
    public byte evaluate(PointsToSetVariable lhs, final PointsToSetVariable[] rhs) {
      ...
      if (constParams != null && constParams[0] != null) {
        ...
        try{//FIXME: JEFF -- may through NPE
        receiverVals = rhs[0].getValue();
        }catch(Exception e){return NOT_CHANGED;}
      }
      ...
      private void rec(final int pi) {
        if (pi == params.length) {
          ...
          if (ik != null) {
            ...
            //bz: can see Implicit pointer key when updating invoke TODO:
            PointsToSetVariable ptsv = system.findOrCreatePointsToSet(var);
            if(ptsv != null){
              IntSet s = ptsv.getValue();
              if (s != null && !s.isEmpty()) {
