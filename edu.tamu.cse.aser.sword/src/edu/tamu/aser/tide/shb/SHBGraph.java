...
//                        return -1;
        ...
        public int whoHappensFirst(SyncNode sync, INode node, int sTID, int nTID) {//node maybe startnode or r/wnode
                ...
                String cgsync = "";
                CGNode syncnode = sync.getBelonging();
                if(syncnode == null) // == null : this is main's start node
                        cgsync = id2CGNode.get(main).getMethod().toString();
                String cgnode = "";
                CGNode nodenode = node.getBelonging();
                if(nodenode == null)
                        cgnode = id2CGNode.get(main).getMethod().toString();
...
//              if(method.contains("< Application, LExampleCSStaticNO, f(LMyClass;)V >"))
//                      System.out.println();
