...
public class SearchThreads {
	...
	public static void main(String[] args) throws Exception {
		...
		final String queryBase = null;
		int repeat = 0;
		final boolean raw = false;
		final String normsField = null;
		final int hitsPerPage = 10;
		final String outBase = null;
		int threads = 1;
		int totalQueries = 32;
...
//		final SearchThreads search = this;
//		ParallelArray<QueryProcessor> processors = ParallelArray.createUsingHandoff(new QueryProcessor[totalQueries],
//				ParallelArray.defaultExecutor());
//		processors.replaceWithMappedIndexSeq(new IntToObject<QueryProcessor>() {
//			@Override
//			public QueryProcessor op(int i) {
//				return new QueryProcessor(search, "Query" + i, i, index, outBase, queryBase, field, normsField, raw,
//						hitsPerPage);
//			}
//
//		});
//
//		processors.apply(new Procedure<QueryProcessor>() {
//
//			@Override
//			public void op(QueryProcessor p) {
//				try {
//					p.run();
//				} catch (IOException e) {
...
//		});
	...
	static class QueryThread extends Thread {
		...
		public QueryThread(SearchThreads parent, String name, int id, int threadCount, int totalQueries, String index,
				String outBase, String queryBase, String field, String normsField, boolean raw, int hitsPerPage) {
			super(name);
			this.parent = parent;
			this.id = id;
		...
		public void run() {
			try {
				...
				for (int i = 0, queryId = id; i < count; i++, queryId += threadCount) {
					// make and run query
					new QueryProcessor(parent, name, queryId, index, outBase, queryBase, field, normsField, raw,
							hitsPerPage).run();
				}
			} catch (Exception e) {
				e.printStackTrace();
	...
	public static class QueryProcessor {
		...
		public QueryProcessor(SearchThreads parent, String name, int id, String index, String outBase, String queryBase,
		 ...
		 * engine presents pages of size n to the user. The user can then go to
		 * the next page if interested in the next hits.
		 * 
		 * When the query is executed for the first time, then only enough
		 * results are collected to fill 5 result pages. If the user wants to
		 * page beyond this limit, then the query is executed another time and
